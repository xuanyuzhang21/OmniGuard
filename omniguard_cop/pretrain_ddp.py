...
parser.add_argument('-a', '--accumulate-steps', type=int, default=1)
...
save_dir = os.path.join(args.save_root, f"{now}_{model_tag}")
os.makedirs(save_dir, exist_ok=True)
pretrained = args.pretrained

# trainstage
trainstage_start = [0] * 4
trainstage_ramp = [0, 0, 10000, 10000]
trainstage_thr = [0.9, 0.95, 0.98, 0]
...
# %%
perturbator_early = A.CenterCrop(
    size=(resize, resize),
...
class TrainStage(IntEnum):
    ...
    def get(cls, idx):
        ...
        return cls._value2member_map_.get(idx, None)
...
if args.test_only:
    trainstage = TrainStage.IMPROVE_IMAGE
    model.eval()
    ...
    with torch.inference_mode():
        for idx, sample in tqdm(enumerate(val_loader)):
            ...
            if trainstage == TrainStage.FIXED_IMAGE: 
                inputs = fixed_image[:secrets.size(0)]
            ...
            # 2. perturbation
            if trainstage >= TrainStage.ANTI_NOISE:
                perturb_w = 1.0
                ...
                reconstructions_perturbed = perturbator(reconstructions, perturb_w, perturb_idxs)
            
            else:
                perturb_w = 0.0
                reconstructions_perturbed = perturbator_early(reconstructions)
    
            # 3. decode
            secrets_logit = model(reconstructions_perturbed, mode='decode')

            if trainstage >= TrainStage.IMPROVE_IMAGE:
                ramp_w = 1.0
    
                cur_image_w =  ramp_w * (image_w * secret_w - 1) + 1
                cur_secret_w = secret_w
            else:
                ramp_w = 0.0
                cur_image_w, cur_secret_w = 1, secret_w
    ...
    trainstage = TrainStage.FIXED_IMAGE
    ...
    for epoch in range(num_epochs):
        ...
        for idx, sample in tqdm(enumerate(train_loader)):
            with accelerator.accumulate(model):
                ...
                if global_step == 0: 
                    fixed_image = inputs
                    fixed_image = accelerate.utils.broadcast(fixed_image)
                if trainstage == TrainStage.FIXED_IMAGE: 
                    inputs = fixed_image[:secrets.size(0)]
                ...
                # 2. perturbation
                if trainstage >= TrainStage.ANTI_NOISE:
                    steps = global_step - trainstage_start[TrainStage.ANTI_NOISE]
                    perturb_w = min(steps / trainstage_ramp[TrainStage.ANTI_NOISE], 1.0)
                ...
                # 4. backward
                if trainstage >= TrainStage.IMPROVE_IMAGE:
                    steps = global_step - trainstage_start[TrainStage.IMPROVE_IMAGE]
                    ramp_w = min(steps / trainstage_ramp[TrainStage.IMPROVE_IMAGE], 1.0)
                ...
                if (global_step + 1) % log_freq == 0:
                    if accelerator.is_main_process:
                        print('-'*32)
                        print(f"[epoch {epoch:03d}, step {idx:03d}] <{trainstage.name}>"
                            f" bit_acc: {all_bit_acc:.3f}"
                            f" image_loss: {all_image_loss:.3f}")
                        print('-'*32)
                        loss_dict['perturb_w'] = perturb_w
                        loss_dict['image_w_ramp'] = ramp_w
                        print(loss_dict)

                if (trainstage != TrainStage.IMPROVE_IMAGE and 
                    all_bit_acc >= trainstage_thr[trainstage]):
                    if accelerator.is_main_process:
                        torch.save(
                            model.module.state_dict(), 
                            os.path.join(save_dir, f"omniguard-cop_epoch{epoch:03d}_stage{trainstage}_last.pth"))
                        print(f"[stage forward] from {trainstage.name} to {TrainStage.get(trainstage + 1).name}")
                        
                    trainstage = TrainStage.get(trainstage + 1)
                    trainstage_start[trainstage] = global_step                
                    best_image_loss, best_bit_acc = float('inf'), 0.0  # reflesh the best metrics
